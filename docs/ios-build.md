...
The iOS build happens **in the cloud on GitHub Actions** â€” no Mac needed. The
...
## Step 1 â€” Trigger the cloud build

1. Open the repo on GitHub â†’ **Actions** tab.
...
> Release** under **Releases â†’ Drafts**.

## Step 3 â€” Install with TrollStore

1. Open **TrollStore** on your iPhone.
2. Tap the **+** button (top right) â†’ **Install IPA File**.
3. Browse to `RankRocketAI.ipa` (use the **Files** app â†’ Downloads).
...
license key from `rpfinserv24@gmail.com`.

## Rebuilding

Any time you change the app code, just re-run the workflow (Step 1) and repeat
Steps 2â€“3. TrollStore keeps your app installed; reinstalling over it preserves
...
| Problem | Fix |
|---------|-----|
| Build fails on GitHub | Open the failed run â†’ logs. Common causes: CocoaPods version, or a stale Flutter lock. Retry once first. |
